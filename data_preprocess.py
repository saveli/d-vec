...
# sort - glob loads in order how fs stored files
# reverse - train folder before test, so changes to orig code is minimal
audio_path = sorted(glob.glob(os.path.dirname(hp.unprocessed_data)), reverse=True)
...
def save_spectrogram_tisv():
    ...
    train_speaker_num = 462  # 0 to 461 are train, 462 to 629 are test
    print("total speaker number : %d"%total_speaker_num)
    print("train : %d, test : %d"%(train_speaker_num, total_speaker_num-train_speaker_num))
    for i, folder in enumerate(audio_path):
        print("%dth speaker processing..."%(i+1))
        utterances_spec = []
        for utter_path in glob.glob(folder+'/*.WAV'):
        ...
        if i<train_speaker_num:      # save spectrogram as numpy file
            np.save(os.path.join(hp.data.train_path, "%s.npy"%speaker), utterances_spec)
        else:
            np.save(os.path.join(hp.data.test_path, "%s.npy"%speaker), utterances_spec)
